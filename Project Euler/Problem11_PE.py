...
# def productDOWN(M[i][j]):
# 	if i>16:
# 		return 1
# 	else:
# 		return M[i][j]*M[i+1][j]*M[i+1][j]*M[i+1][j]	

# def productLEFT(M[i][j]):
# 	if i<3:
# 		return 1	
# 	else:
# 		return M[i][j]*M[i][j-1]*M[i][j-2]*M[i][j-3]

# def productRIGHT(M[i][j]):
# 	if i>16:
# 		return 1	
# 	else:
# 		return M[i][j]*M[i][j+1]*M[i][j+2]*M[i][j+3]				

# def productDIAGONAL(M[i][j]):
# 	if i>16 or j>16:
# 		return 1
# 	else:
...
# for i in range(20):
# 	for j in range(20):
# 		calculatedMax = max(productUP(M[i][j]), productDOWN(M[i][j]), productLEFT(M[i][j]), productRIGHT(M[i][j]), productDIAGONAL(M[i][j]))
# 		if  calculatedMax > maximum:
